...
*/
#include <FastLED.h> 
#define NUM_LEDS 400        // Enter the number of pixels i.e. No. of strings x 50
...
void setup()
{
  ...
  FastLED.addLeds<WS2811, 4, GRB>(leds, 100, 150).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, 5, GRB>(leds, 150, 200).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, 6, GRB>(leds, 200, 250).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, 7, GRB>(leds, 250, 300).setCorrection( TypicalLEDStrip );  
  FastLED.addLeds<WS2811, 8, GRB>(leds, 300, 350).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, 9, GRB>(leds, 350, 400).setCorrection( TypicalLEDStrip );
...
{  // 5 channels are coming in to the Arduino
 // Set some counter / temporary storage variables
  ...
  for(;;) {
    ...
    FastLED.addLeds<WS2811, 4, GRB>(leds,100, 150);
    FastLED.addLeds<WS2811, 5, GRB>(leds,150, 200);
    FastLED.addLeds<WS2811, 6, GRB>(leds,200, 250);
    FastLED.addLeds<WS2811, 7, GRB>(leds,250, 300);
    FastLED.addLeds<WS2811, 8, GRB>(leds,300, 350);
    FastLED.addLeds<WS2811, 9, GRB>(leds,350, 400);
